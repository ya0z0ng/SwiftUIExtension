...
//  swiftUIExtension
//
//  Copy and Created by Renaud Wang on 2/8/25.
//
/// [CachedAsyncImage](https://github.com/lorenzofiamingo/swiftui-cached-async-image)
/// Original Created by Lorenzo Fiamingo on 04/11/20.
//
...
public struct EAsyncImage<Content>: View where Content: View {
...
private class URLSessionTaskController: NSObject, @unchecked Sendable, URLSessionTaskDelegate {
...
///  Loads and displays an image from the specified URL and cache to disk with 10G limited
///
///  - Parameters:
///   - urlString: The URL string of the image to display.
///   - scale: The scale to use for the image. The default is `1`. Set a
///         different value when loading images designed for higher resolution
///         displays. For example, set a value of `2` for an image that you
///         would name with the `@2x` suffix if stored in a file on disk.
///
@available(iOS 15.0, macOS 12.0, tvOS 15.0, watchOS 8.0, *)
public extension EAsyncImage {
    init(_ urlString: String?, scale: CGFloat = 1) where Content == _ConditionalContent<Image, Color> {
        let cache = URLCache(memoryCapacity: 0, diskCapacity: 10_000_000_000)
        self.init(url: URL(string: urlString ?? ""), urlCache: cache) { image in
            image.resizable()
        } placeholder: {
...
protocol EAsyncImageStyle {
    associatedtype Body: View
    typealias Configuration = EAsyncImageStyleConfiguration
    func makeBody(configuration: Self.Configuration) -> Self.Body
}

struct EAsyncImageStyleConfiguration {
    let content: AnyView
}

struct AvatarEAsyncImageStyle: EAsyncImageStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.content.clipShape(.circle)
    }
}

struct BasicEAsyncImageStyle: EAsyncImageStyle {
    func makeBody(configuration: Configuration) -> some View {
...
enum EAsyncImageStyles {
    case avatar, basic
    
    func get() -> any EAsyncImageStyle {
        switch self {
            case .basic: return BasicEAsyncImageStyle()
            case .avatar: return AvatarEAsyncImageStyle()
        }
    }
}

struct EAsyncImageStyleKey: @preconcurrency EnvironmentKey {
    @MainActor static var defaultValue = EAsyncImageStyles.basic
}

extension EnvironmentValues {
    var EAsyncImageStyle: EAsyncImageStyles {
        get { self[EAsyncImageStyleKey.self] }
        set { self[EAsyncImageStyleKey.self] = newValue }
    }
}

extension View {
    func imageStyle(_ style: EAsyncImageStyles) -> some View {
        AnyView(style.get().makeBody(configuration: EAsyncImageStyleConfiguration(content: AnyView(self))))
    }
}
...
#Preview {
    if #available(iOS 15.0, *) {
        EAsyncImage("https://gips3.baidu.com/it/u=1022347589,1106887837&fm=3028&app=3028&f=JPEG&fmt=auto?w=960&h=1280")
